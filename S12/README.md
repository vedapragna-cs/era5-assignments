...
| Asked for | Where |
|---|---|
| 32 virtual GPUs, runnable on a laptop or Colab | `VirtualCluster(32, gpus_per_node=8)`, notebook §3 |
| A demo model running on them | residual MLP, 545,546 params, 10 units, notebook §4–5 |
| Simulate ZeRO-1/2/3 and show memory + computation changes | notebook §5.1–5.3: measured memory, per-step memory timeline, communication volume |
| Make sure it matches what ZeRO does | notebook §6: bit-identical weights, formulas, paper Table 1, the lecture's 30B numbers, an OOM demo, communication time |
| Explain the stages and their pros and cons | notebook §1 and §7, summarised below |
...
ZeRO-1 and ZeRO-2 flatten out at 4Ψ and 2Ψ, because their unsharded part never shrinks. **Only ZeRO-3 keeps scaling as 1/N**, which is what makes a model larger than one GPU trainable at all. On 32 × 80 GB, the largest model states that fit are roughly 5B (DP), 18B (ZeRO-1), 33B (ZeRO-2) and 160B (ZeRO-3).
...
| **ZeRO-3** | Everything scales as 1/N; trains models bigger than a single GPU | **1.5× communication**, many small all-gathers, gather spikes, most tuning, very sensitive to inter-node links | The weights themselves don't fit. Best with a fast interconnect, or kept within a node |
